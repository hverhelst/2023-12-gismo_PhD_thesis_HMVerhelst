...
int main(int argc, char *argv[])
{
    ...
    //gsFunctionExpr<> f("(cos(4*pi*x) - 1) * (cos(4*pi*y) - 1)",2);
    gsFunctionExpr<> f("2*(0.5-x)^2*(0.5-y)",2);
    //gsFunctionExpr<> f("2*(0.5-x)",2);
    gsInfo << "Function of interest: " << f << "\n";
